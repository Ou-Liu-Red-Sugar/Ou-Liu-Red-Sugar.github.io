...
date: 2026-02-28
tags: ["monthly-review", "process", "allocation", "lessons"]
hideMeta: true
ShowToc: true
TocOpen: false
---
...
### 1.2 The valuation case for rotation
...
The S&P 500 finished February slightly negative (−0.43% MTD). The Nasdaq posted its worst month since March. Under the surface, a major rotation was playing out: energy, materials, and consumer staples led, while tech lagged. Gold rose on tariff uncertainty (Trump's 15% global tariffs) and mounting geopolitical tension. On Feb 28, the U.S. and Israel launched strikes on Iran, dramatically escalating the situation into March.

### 1.4 My mental state

I was calmer this month than in January. The software selloff created excitement, but my actions were more deliberate: I had a clear thesis (expensive beta → discounted quality), and the execution was staged over multiple days. The main emotional pressure was from PYPL — deciding to realize a large loss is never comfortable, but it was the right call.
...
- **MTD TWR: +1.88%**
- Benchmarks (approximate Feb): SPX −0.4%, NDX −2.5%, Dow +1.9%

The portfolio outperformed the S&P 500 mainly because of gold and defensive holdings (KO, KR, PGR rebound), which offset losses in MSFT and the newly opened tech positions.
...
### 3.1 The valuation rotation: VOO/VEU → MSFT/SPGI

**The thesis:** SPX PE ~29x is historically expensive. Rather than holding expensive beta through VOO, rotate into quality individual names that the software selloff has made significantly cheaper.

**Sold:**
- VOO: 5 shares sold (7→2) — SPX PE ~29x; retained 2 shares as residual core
- VEU: all 16 shares sold — limited upside vs. software-mageddon opportunities

**Kept:**
- RSP: all 24.3 shares — equal-weight PE ~20x, more reasonable valuation
...
### 4.1 Sleeve allocation (Jan → Feb)

| Sleeve | Jan End | Feb End | Change | Band |
|--------|---------|---------|--------|------|
| Core ETFs | 28.4% | **17.9%** | −10.5% | 35–60% |
| Stocks | 53.5% | **63.2%** | +9.7% | 20–50% |
| Gold | 14.9% | **15.3%** | +0.5% | 8–15% |
| Cash | 3.2% | **3.5%** | +0.3% | 5–30% |

### 4.2 Honest assessment of the band violations

The Playbook bands are violated. The question is **whether the bands need updating** or whether I need to rebalance back.

**The case for "this was correct":**
- VOO at SPX PE ~29x is not "safe core" — it's expensive beta
- The software selloff created genuine value in quality names
- RSP was kept as the core position with better valuation
- The rotation should be judged over 6–12 months, not at month-end

**The case for "this was still a violation":**
- The Playbook bands exist precisely to prevent smart-sounding rationalizations
- Every opportunity looks "unique" in the moment
- 63% in individual stocks requires my stock-picking to be consistently good — and I have ~6 months of experience
- The bands should be fixed via cash flows, not ignored

**My conclusion:** The rotation thesis was defensible, but the magnitude was too aggressive. I should have sold less VOO (e.g., keep 4–5 shares instead of 2) and bought less SPGI (e.g., 7 shares instead of 11). The correct fix now is cash-flow rebalancing + gold trimming, not panic reversal.

### 4.3 Top positions (concentration)

| Rank | Ticker | Weight | Sleeve |
|------|--------|--------|--------|
| 1 | GLDM | 15.3% | Gold |
| 2 | PGR | 13.4% | Stocks (Defense) |
| 3 | RSP | 10.5% | Core ETF |
| 4 | SPGI | 10.2% | Stocks (Opportunity) |
| 5 | MSFT | 9.1% | Stocks (Opportunity) |

Software-adjacent exposure (MSFT + SPGI + ACN) = 21.1%. This is an implicit sector bet that I should monitor, even if each position was entered with its own thesis.

---

## 5) What went wrong

### 5.1 Magnitude of the rotation

The rotation direction was correct (expensive beta → discounted quality), but the size was too aggressive. Selling 5 of 7 VOO shares + all VEU in a single month moved too much capital at once. A more measured approach: sell 3 VOO, keep VEU, build SPGI and MSFT more slowly.
...
### 6.1 The rotation thesis

Selling expensive beta (SPX PE ~29x) to buy discounted quality names during a sector-specific selloff is a defensible active management decision. The key names (MSFT, SPGI) are wide-moat companies experiencing a sentiment-driven selloff, not a fundamental breakdown. Multiple analysts (Morningstar, J.P. Morgan) noted the same opportunity.
...
### 7.1 Valuation rotation is valid, but magnitude matters

The direction was right; the size was too large. In the future: rotate gradually, keep more of the core intact, and accept that I might participate less in an opportunity.

### 7.2 "Staged entry" needs a time dimension

Price-staging without time-staging is not discipline. New rule: minimum 5 trading days between adds.

### 7.3 Sector concentration builds invisibly

MSFT + SPGI + ACN = 21% in software/data names. Each position had its own thesis, but together they create a sector bet. New monitoring: check total sector exposure before any add.

### 7.4 Capitulation is expensive but educational

PYPL taught me: if a position lacks a clear thesis and keeps grinding, the sizing was wrong from the start. Cut before it becomes emotionally sticky.

---

## 8) Rule updates (process changes after this month)

### 8.1 Valuation-based rotation clause (new)

Selling Core ETFs is allowed if the valuation case is explicit and written:
- Must state the valuation metric (e.g., "SPX PE ~29x, well above 20-year avg of ~16x").
- Must identify what is being bought and why it offers better risk/reward.
- Must keep at least one core ETF at ≥10% weight (currently RSP fills this role).
- The rotation must be **gradual** (spread over at least 2–3 weeks).

This replaces a blanket "never sell Core" rule with a conditional rule that requires explicit justification.

### 8.2 Time-staged entries (minimum gap)

Staged entries must have at least **5 trading days** between steps. Same-day or next-day adds are separate lots but not separate "steps."

### 8.3 Sector concentration monitoring

Before adding to any position, check total GICS sector exposure. Soft cap: **25%** in any single sector. If above 25%, no further adds until it drops below.

### 8.4 Rebalance plan for March

**Priority 1:** All new cash → Core ETFs (RSP or VOO if SPX PE improves) until Core ≥ 25%.
**Priority 2:** Trim gold (GLDM) on the Iran spike — take partial profits while gold is elevated, redirect proceeds to Core ETFs. This executes the "partial profit rule" from the January lesson.
**Priority 3:** Do not add to any stock position until the stock sleeve drops below 55%.
**PGR:** Hold — not yet at target price. Continue monitoring.

---

## 9) Forward look: March and the Iran situation

### 9.1 Geopolitical shock (Feb 28 – March)
...
**TLN (3.9%):** Energy/power benefits from oil price increases. Hold.
**KO, KR (13.2% combined):** Consumer staples are defensive in geopolitical crises. Hold.
**PGR (13.4%):** Insurance is relatively insulated from Middle East risk. Not yet at target price — hold.

**MSFT, SPGI, ACN (21.1% combined):** Already under pressure from the software selloff. A risk-off environment could extend the downturn, but these are long-term holds. The thesis is valuation-driven, not timing-driven. Do not panic-sell.

### 9.3 What I should do (and not do)

**Do:**
- Trim gold on strength → redirect to Core ETFs.
- Use any new cash for Core ETFs.
- Stay calm — the portfolio has natural hedges (gold, staples, insurance).

**Do not:**
- Do not try to "trade" the Iran situation.
- Do not sell software positions into a geopolitical panic unrelated to their fundamentals.
- Do not abandon the rebalancing plan because "the market changed."

### 9.4 Scenario thinking

| Scenario | Probability (guess) | Portfolio impact | Action |
|----------|-------------------|-----------------|--------|
