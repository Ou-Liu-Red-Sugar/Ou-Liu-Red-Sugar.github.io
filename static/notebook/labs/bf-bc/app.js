...
function bridgeSVG(id,title,opening,steps,closingLabel){
 ...
 let g='<svg class="bridge-svg" viewBox="0 0 900 292" role="img" aria-labelledby="'+id+'-title"><title id="'+id+'-title">'+esc(title)+'. 首尾柱是总额，中间柱显示有符号变化. 精确数字见下面同源表.</title><line x1="30" y1="'+Y(0)+'" x2="875" y2="'+Y(0)+'" stroke="#657681"/><text x="30" y="22" font-size="13"> M 美元；中间柱为变化，首尾柱为总额</text>';
...
function cashWidget(){const el=$('#EXP-BF07-CASH-BRIDGE');const draw=()=>status(el,()=>{const mode=el.querySelector('[name=industry]').value,y=number(el.querySelector('[name=year]')),t=D.tables[{software:'sf_cash',retail:'cost_cash',bank:'jpm_cash'}[mode]],p=E.cashBridge(t,y);let msg=mode==='bank'?'现金集合：cash and due from banks and deposits with banks. 交易资产与待售贷款是经营对象，客户存款变动在筹资.':'现金集合：cash and cash equivalents.';if(mode==='software')msg+=' 资产负债变化扣除企业合并；D&A包括取得无形资产、固定资产及使用权资产相关项目.';
 let html='<p>'+esc(t.title+' · '+t.periods[y])+' · M 美元</p><p>'+esc(msg)+'</p>'+bridgeSVG('cash-bridge','期初至期末现金桥',['期初',p.begin],[['经营',p.cfo],['投资',p.cfi],['筹资',p.cff],['汇兑',p.fx]],'期末')+table(['现金桥','金额'],[['期初',p.begin],['经营净现金',p.cfo],['投资净现金',p.cfi],['筹资净现金',p.cff],['汇率影响',p.fx],['计算期末',p.end],['原表期末',p.reported_end]]);
...
function equityWidget(){const el=$('#EXP-BF08-EQUITY-SHARES');const draw=()=>status(el,()=>{const p=E.equity(D),mode=el.querySelector('[name=view]').value;let h;
 if(mode==='equity'){h=table(['FY2026权益桥： M 美元','有符号金额'],[['期初',p.opening],...D.sf_equity.rows.slice(8,15).map(r=>[r[1],r[9]]),['期末',p.closing]]);}
 else if(mode==='shares'){h=table(['FY2026数量桥： M 股','金额'],[['期初发行',p.issued_begin_m],['期初库存股（减）',-p.treasury_begin_m],['期初流通',p.outstanding_begin_m],['当期发行（不能全称SBC）',p.issued_end_m-p.issued_begin_m],['当期回购',p.treasury_begin_m-p.treasury_end_m],['期末流通',p.outstanding_end_m]])+'<p>原表 M 取整的0不是无任何交易. 回购股数不是现金金额.</p>';}
 else {const den=el.querySelector('[name=denominator]').value;h=table(['分母用途',' M 股','7457／分母'],[['报告基本EPS',p.basic_shares_m,p.basic_eps],['报告稀释EPS',p.diluted_shares_m,p.diluted_eps],['错误口径对照：期末股数，不是报告EPS',p.outstanding_end_m,D.sf_eps.net_income[0]/p.outstanding_end_m]])+'<p class="result">当前选择：'+esc(den==='wrong'?'期末股数：不能当作报告EPS':den==='basic'?'基本EPS，期间加权平均':'稀释EPS，按Note 13工具规则')+'</p><p>另有反稀释奖励9 M 股，被排除于本期稀释分母；不得再加.</p>';}
...
equityWidget();
function evidenceWidget(){const el=$('#EXP-BF09-EVIDENCE');const texts={statement:'主表：Salesforce FY2026收入41525 M 美元. 主表给合并数和期间，不能识别每份合同的履约义务或独立售价.',policy:'Note 1：控制权、可区分履约义务和相对单独售价. 政策告诉我们判断框架，实际合同证据仍需取得.',audit:'Revenue Recognition CAM：尤其困难、主观或复杂的审计判断与应对程序. 不是单独负面审计意见，不给公司打分.'};
