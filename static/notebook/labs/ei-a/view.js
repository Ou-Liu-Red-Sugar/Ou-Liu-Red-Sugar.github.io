(function (root, factory) {
  ...
  function actualMachineryTable(data) {
    ...
    return table(['工程机械字段', ...record.rows.map(r => r.month + (r.status_in_selected_table === 'preliminary' ? ' p' : r.status_in_selected_table === 'revised' ? ' r' : ''))], fields.map(([key, label]) => [label, ...record.rows.map(r => n(r[key]))]), '同一期 Census M3，2026-07-02 发布；百万美元，季调、未扣价格变化。p 为初值，r 为本版修订值。');
  }
  function power(p) {
  ...
  function orders(data, p, observedMode = true) {
    ...
    return `${svg(observedMode ? 'orders-real' : 'orders-toy', 850, 355, '未交订单的存量流量桥', formula + '，百万美元。库存单列，不作为余额桥中的订单流。', txt(74, 24, '未交订单及桥接金额（百万美元）', 'axis-label') + content, observedMode ? '同版统计定义核对：季调新订单 N=S+ΔB；381不是三个独立观测互证。代数核对差为0不能推断经济取消、修改或调整为零。' : '这屏使用一般企业内部台账的教学变式，不套用 Census 新订单构造。净额模式已扣取消，总额模式才从新增总额中减取消。', true)}<p class="formula">期末未交订单 = ${formula}（百万美元）</p><div class="metric-grid">${metric('未交订单变动', sign(r.change), '百万美元', true)}${metric('期末存货（另一个状态）', n(r.inventory), '百万美元')}${metric('期末库存 / 当月出货', n(r.inventoryRatio, 7), '金额存量与月度金额流量之比')}</div><p class="caption">库存没有作为订单桥的一笔流入，也没有从库存画出“导致交期”的箭头。这个比值没有逐单日期和实物数量，不能直接读取为客户平均交期。</p>${!observedMode ? `<p class="result-line">${p.orderMode === 'net' ? `当前新订单已是净额 ${n(r.net)}；取消 ${n(p.cancel)} 已包含在内，不再扣减。` : `当前新增总额 ${n(p.Ngross)} − 取消 ${n(p.cancel)} = 净新增 ${n(r.net)}。`}</p>` : `<p class="source-line">当前材料月份 ${esc(r.month)}，状态 ${r.status === 'preliminary' ? '初值 p' : '修订值 r'}；期初来自同版 ${esc(r.previousMonth)} 期末。</p>`}`;
  }
  ...
  function metadata(data) {
    ...
    const cards = [
      { key: 'eia', title: 'EIA · 年度最终客户售电', source: 'EI-S04-EIA-2024', fields: [ ['测量对象', '居民最终客户售电、收入及公布均价'], ['地理覆盖', '美国 50 州与华盛顿特区'], ['单位', '千 MWh；百万美元；美分 / kWh'], ['频率', '年度'], ['期间', '2023 / 2024'], ['存量 / 流量', '售电和收入是期间流量；均价为收入 / 电量口径'], ['季节调整', '所用年度原表未标季调'], ['价格调整', '收入与公布均价为金额 / 价格口径；未提供匹配的实际收入序列'], ['发布日', '2025-10-16'], ['固定版本', 'Electric Power Annual with data for 2024；final'], ['读取状态', '2026-09-21；Tables 2.5—2.7 及表注已核'] ], raw: actualElectricity(data), explanation: `<p class="formula">${n(e.rows[1].residential_revenue_million_USD)} × 10⁶ USD ÷ (${n(e.rows[1].residential_sales_thousand_MWh)} × 10³ MWh × 10³ kWh/MWh) × 100 cent/USD<br>= <strong>${n(e.rows[1].reconstructedPrice, 7)} 美分 / kWh</strong></p><p>电量原数值换算后也是百万 kWh，先相除得到美元 / kWh，再乘 100。公布值 16.48 与复算值因显示位数略有差别；均价并非某位客户多用一单位电的边际电价。</p>` },
      { key: 'census', title: 'Census M3 · 2026 年 5 月工程机械', source: 'EI-S06-M3-202605', fields: [ ['测量对象', '制造商出货、净新订单、未交订单和存货'], ['地理覆盖', '美国制造调查中的工程机械行'], ['单位', '百万美元'], ['频率', '月度'], ['期间', '同一报告中的 2026-04 / 05'], ['存量 / 流量', '出货 / 新订单为期间流量；未交订单 / 存货为期末余额'], ['季节调整', '是（含适用的节假日、交易日与季节处理）'], ['价格调整', '未扣价格变化；不能直接转成设备台数'], ['发布日', '2026-07-02 10:00 EDT'], ['固定版本', 'CB26-104 M3-2(26)-05；5 月 p、4 月 r'], ['读取状态', 'Lead 已取得固定历史 PDF，核第 1、6—9 页；原数据与图注按同版保留'] ], raw: actualMachineryTable(data), explanation: '<p>新订单已经扣除取消。季节调整不等于扣通胀；月末库存与月内出货即使同为美元，也承担不同的存量 / 流量职责。</p><p>固定发布日期和 p / r 标签一起定义这次资料快照，后来的修订不会覆盖这里的历史版本。调查面板不是概率抽样；精确读取公布单元格，并不表示总体抽样误差已经量化。</p>' },
