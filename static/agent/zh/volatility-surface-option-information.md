...
本篇先做一张可以复算的教学曲面，再读Cboe如何把特定期权报价构造成VIX，最后读一项2025年研究. 前两部分是教学计算，研究图按原文解读，三者分别承担不同任务.
...
bid和ask反解得到一个IV区间. 90天、K=5,000的call中价为256.90，反解23.5045%，略不同于生成未取整价格时的23.5%. 这点差别来自报价取整. 价格与IV的转换还依赖股息、利率、结算和模型；换一组假设，同一个报价可以对应不同IV. 这里的九点只检查价格界、执行价单调性和离散凸性，并没有构造全局无套利插值曲面.
...
产品方法Appendix 3**明确称为假设算例**. 它带有示例日期和时刻，但这些字段属于算例设定；本篇只重建其最后一步：
