...
渐进 $H^2$ 密度、Doob $L^2$ 最大估计、连续版本、停止规则与局部积分构造.

Entry: zh-qt16p1 | Node: QT16-P1 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
读者为有充分数学背景的高年级本科生至研究生. 先实际读取 agent_packet.required_readings 指定完整单元，选择可选分支后再读 optional_readings；记录题名、版本、定位与支持内容. 缺失必读单元时先取得等价原件，再解释依赖它的命题. 经截断、向后平均和左阶梯建立渐进过程密度；证明Doob最大估计，构造共同连续积分版本，传递等距、停止规则和局部化，区分两种等价关系. 用完整推导或计算诊断理解，已掌握步骤直接继承，再用改变条件的任务检验迁移. runtime_reading_log记录实际读取.
...
```json
{
  ...
  "required_readings": [
    {
      ...
      "required_unit": {
        "locator": "§1.1–1.2 pp.1–4；§2.1–2.2 pp.4–5；§3.1–3.5 pp.7–13",
        "scope": "密度、连续完成、停止与局部化完整单元",
        "purpose": "逐步核本站 Doob4/版本/停止规则补全"
      },
  ...
  "runtime_reading_log": [],
  "supplied_inputs": {
    "competence": "QT16 简单积分/等距；L2 完备、Lebesgue 微分、Tonelli/DCT、条件期望收缩.",
    "static_equivalent": "本篇 reader 全部默认表、证明及题解；HTML 禁用脚本仍可读.",
    "attachments": [
      {
        "title": "本篇完整静态阅读、全部题解与证明",
        "uri": "https://ou-liu-red-sugar.github.io/notebook/labs/qt-ghi/static/QT16-P1.html",
        "kind": "html"
      },
      {
        "title": "本篇同源 Markdown",
        "uri": "https://ou-liu-red-sugar.github.io/notebook/labs/qt-ghi/static/QT16-P1.md",
        "kind": "markdown"
      }
    ],
    "proof_scope": "本篇完整证明为 supplied site body；通常条件、progressive、dt⊗P 等价类、共同满概率版本与局部/全局矩条件按正文."
  },
  "prompt": "读者为有充分数学背景的高年级本科生至研究生. 先实际读取 agent_packet.required_readings 指定完整单元，选择可选分支后再读 optional_readings；记录题名、版本、定位与支持内容. 缺失必读单元时先取得等价原件，再解释依赖它的命题. 经截断、向后平均和左阶梯建立渐进过程密度；证明Doob最大估计，构造共同连续积分版本，传递等距、停止规则和局部化，区分两种等价关系. 用完整推导或计算诊断理解，已掌握步骤直接继承，再用改变条件的任务检验迁移. runtime_reading_log记录实际读取.",
  "learning_task": "经截断、向后平均和左阶梯建立渐进过程密度；证明Doob最大估计，构造共同连续积分版本，传递等距、停止规则和局部化，区分两种等价关系.",
...
固定 $T<\infty$，滤过满足通常条件（滤过右连续，且 $\mathcal F_0$ 包含 $\mathcal F$ 中所有 $P$ 零集及其子集）；$W$ 是相对于该滤过的 Brownian 运动. 渐进可测过程在每个 $[0,u]\times\Omega$ 上对 $\mathcal B([0,u])\otimes\mathcal F_u$ 可测. 记
...
为每个被积过程等价类构造连续平方可积鞅 $I(H)$，使

$$
\mathbb{E}|I_t(H)|^2=\mathbb{E}\int_0^tH_s^2ds.
$$

渐进可测空间按这个范数是闭的 $L^2$ 子空间：取 $L^2$ 收敛列的几乎处处收敛子列，其逐点上极限在存在有限极限处等于极限，在其余点置零，仍渐进可测，因而给出该 $L^2$ 极限的合法代表. [^construction]
...
逐时点的极限可有不同零测例外. 路径最大差估计将给出共同满测度事件上的连续极限.
...
可积性、适应性已经得到，所以它是鞅；线性与零均值也由同一极限传递. [^completion]
...
最后对一般 $H$ 取简单逼近 $H^n$. 乘截停指标不会增大 $\mathcal H_T^2$ 距离；左边的误差被未停止过程的最大误差控制. 于是两边再次在最大均方范数下传递，得到停止规则. [^stop]
...
因 $A_T<\infty$ a.s.，每条路径在充分大的 $m$ 下均有 $\tau_m=T$，拼接的积分最终稳定. 各停止过程为平方可积鞅，拼接结果为连续局部鞅.
...
每个固定 $t$ 满足 $P(U=t)=0$，但每条样本路径在 $t=U$ 有尖点，故所有时点相等的概率为0. 该过程不连续，无法从稠密时点的相等延伸到所有时点.

<div data-experiment-slot="VIEW-QT16-P1-PROOF"></div>

[^construction]: Lalley，[Notes on the Itô Calculus](https://www.stat.uchicago.edu/~lalley/Courses/385/Old/ItoIntegral-2012.pdf)，2012-05-15，§1.1–1.2 的渐进可测设置、§3.1–3.2 的简单积分与终点延拓.
[^density]: 同讲义 Lemmas 4–5，pp.3–4.
[^completion]: 同讲义 §2.1–2.2，pp.4–5，连续鞅空间的完备性与最大估计背景.
[^stop]: 同讲义 §3.4–3.5，pp.12–13，停止规则与局部化.

## Experiment inputs and static equivalents
```json
[
  {
    "id": "VIEW-QT16-P1-PROOF",
    "title": "积分构造的证明依赖",
    "anchor": "qt16p1-exercise",
    "description": "共享输入的本篇视图；不新增模拟实验.",
    "inputs": {
      "uri": "https://ou-liu-red-sugar.github.io/notebook/labs/qt-ghi/data/learning-inputs.json",
      "json_pointer": "/QT16-P1",
    ...
    "static_equivalent": "https://ou-liu-red-sugar.github.io/notebook/labs/qt-ghi/static/QT16-P1.html#qt16p1-exercise",
    "result_attachment": {
...
[
  ...
  {
    "from": "qt16p1-terminal",
    "relation": "requires",
    "to": "qt16-simple",
    "reason": "该段实际使用所列局部能力.",
    "required_competence": "简单可预测积分定义"
  },
  {
    "from": "qt16p1-terminal",
    "relation": "requires",
  ...
  },
  {
    ...
    "locator": "Lalley，[Notes on the Itô Calculus](https://www.stat.uchicago.edu/~lalley/Courses/385/Old/ItoIntegral-2012.pdf)，2012-05-15，§1.1–1.2 的渐进可测设置、§3.1–3.2 的简单积分与终点延拓. 本文采用有限 $T$，并将被积过程等价类与连续积分版本分开.",
  ...
  },
  {
    ...
    "to": "QGHI-LALLEY-ITO12",
    "reason": "对应本段作者已著明脚注；教学模型及本站补全证明的身份沿原脚注保留.",
    "locator": "同讲义 Lemmas 4–5，pp.3–4. 向后平均与左阶梯逼近按此完整单元展开；“仅适应”不能替代联合可测性.",
    "scope": "只支持此处脚注具名的定义、条件、证明或原表单元.",
    "citation_labels": [
      "density"
    ]
  ...
  {
    "from": "qt16p1-completion",
    "relation": "supported_by",
    "to": "QGHI-LALLEY-ITO12",
    "reason": "对应本段作者已著明脚注；教学模型及本站补全证明的身份沿原脚注保留.",
    "locator": "同讲义 §2.1–2.2，pp.4–5，给出连续鞅空间完成的背景；本文独立写出所用 Doob 常数 4 和最大均方过程完成步骤.",
    "scope": "只支持此处脚注具名的定义、条件、证明或原表单元.",
    "citation_labels": [
      "completion"
    ]
  ...
  {
    "from": "qt16p1-stopping",
    "relation": "supported_by",
    "to": "QGHI-LALLEY-ITO12",
    "reason": "对应本段作者已著明脚注；教学模型及本站补全证明的身份沿原脚注保留.",
    "locator": "同讲义 §3.4–3.5，pp.12–13，停止与局部化. 本文补齐简单情形、上方网格停时、一般被积过程三次传递，不使用未证明的连续鞅表示.",
    "scope": "只支持此处脚注具名的定义、条件、证明或原表单元.",
    "citation_labels": [
      "stop"
    ]
