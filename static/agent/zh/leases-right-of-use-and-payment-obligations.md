...
Entry: zh-bf13 | Node: BF-13 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
读者为有充分数学背景的高年级本科生至研究生. 你正在教BF-13《租赁、使用权与付款义务》，材料版本2026-09-22. 学习任务：选Costco或Salesforce，恢复两类负债并与到期表对接，辨认费用和现金的不同范围. 先确认所选主分支（默认retail）；required_readings只读common及该分支，all才读取全部主分支，optional仅在启用后读取. 讲解前先实际打开指定公开原文单元，分页读到单元结束，必要表图核原页，记录标题、版本、位置、范围与当前用途；runtime_reading_log从空数组开始，不以文献目录、工具标题或先前已读记录代替本次读取. 读取失败先找同版本正式等价原文；仍缺则明确该单元缺口，不以摘要补讲. 诊断：retail恢复Costco 2024负债2,554/1,498与2025到期表并解释618与460范围；software恢复Salesforce 2025融资资产812与负债678，核2026的569−34=535，并说明转租与约4.4bn更广承诺为何不进入该负债闭合. 先由所选公司到期表恢复两类负债，再找到使用权在合并表中的位置. 沿本包正文、实际输入、静态实验与题解推进，先让读者解释或计算再反馈；已会步骤跳过. 默认用所选公司同一原件比较期迁移；跨公司对照启用后再补读另一公司完整单元. 通过尺度：Costco 618和460不做无调节桥；Salesforce分支默认用2025比较列，不强制迁移到Costco. 保持真实披露、分析计算、教学模型三种身份，参数只改变模型，不反写真实公司数据.
...
## Supplied entry
租赁形成使用能力与未来付款义务. 使用权资产、租赁负债、未来未折现付款、期间费用和现金分别记录这项安排的不同侧面.
...
按Topic 842，合同必须在一段期间内以对价换取对<strong>已识别资产</strong>使用的控制权；客户需能够取得该资产使用产生的几乎全部经济利益，并有权决定资产如何使用. 不控制已识别资产的云服务或外包合同不因按月付款而成为租赁. [^bf13-fasb]

美国租赁准则将使用权和义务列入资产负债表，并区分经营租赁与融资租赁. 融资租赁分别确认使用权摊销和利息费用，偿还本金通常列筹资现金流，利息列经营现金流；经营租赁一般按直线法确认单一租赁费用，现金列经营活动. 短期租赁豁免的政策选择及部分可变付款会使费用范围超出租赁负债的计量范围. [^bf13-fasb]

使用权随服务消耗、减值等变化；负债随利息计量、付款和合同变化而变化. 起点的预付租金、初始直接成本及租金优惠等还会调整使用权资产，因此后续两侧余额可以不同. 本文直接使用原附注给出的付款与负债连接.
...
(1) 融资租赁资产列入合并表的 `Other long-term assets`；(2) 两类流动负债列入 `Other current liabilities`；(3) 长期融资租赁负债列入 `Other long-term liabilities`.

经营租赁负债为 $208+2,460=2,668$，融资租赁负债为 $78+1,401=1,479$. 门店、土地、设施或设备的使用权构成长期经营能力，流动部分对应下一年度付款安排.
...
经营租赁未来未折现付款3,591，减利息923，得到负债2,668；融资租赁为 $2,411-932=1,479$. 合计6,002按各到期年度分布，Thereafter汇总第五年之后的多年付款.

经营租赁付款未扣预计未来转租收入92；已签署但尚未开始的租赁付款1,094位于这张到期表之外. 转租收款和未开始租赁承诺需另列时间与条件.
...
经营租赁现金255列经营活动；融资租赁利息58列经营活动，本金147列筹资活动. 费用618包含可变费用，现金460仅覆盖纳入负债计量的付款，两者之间缺少同范围的应计调节.

本年通过新租赁或租赁修改非现金取得的经营、融资租赁资产分别为294和131.

### 零售分支练习

<strong>任务. </strong>恢复 2024 年的经营与融资租赁负债，并核对负债总额；再说明 2025 年付款到期表的哪个量可以直接和 2025 年负债对接.

<strong>解析. </strong>2024经营负债 $179+2,375=2,554$，融资负债 $147+1,351=1,498$，合计4,052. 2025到期付款减表列利息后得到2,668和1,479，分别与当年两类负债对应.
...
## 3. Salesforce租赁资产与期限结构

Salesforce FY2026截至2026-01-31，比较日为2025-01-31. 租赁涉及办公室、数据中心和设备；融资租赁资产包含在PP&E内，融资租赁负债列入相应流动及非流动其他负债. [^bf13-crm]
...
单位：M 美元. 融资租赁资产净额为 $1,427-813=614$，负债为 $275+260=535$；两者分别沿摊销和偿付等关系变动.
...
经营租赁 $3,075-338=2,737$，融资租赁 $569-34=535$，分别对应原表负债.

到期表中的经营租赁付款未扣未来转租收入，后者在未来五年约258 M 美元、之后约35 M 美元. 包含尚未开始租赁的承诺总额约4.4 B 美元，其中约3.8 B 美元与办公室及数据中心有关. 4.4 B的范围与精度均不同于到期表的3,075 M，无法据此取得精确的未开始租赁金额. [^bf13-crm]

FY2026经营租赁费用614，融资使用权摊销375、利息25；纳入负债计量的现金为经营租赁695、融资租赁利息25、本金367，合计1,087. 本金付款减少负债，使用权摊销分配资产成本，因而费用与付现沿不同关系变动. [^bf13-crm]

### 软件分支练习

<strong>任务. </strong>用 2025-01-31 比较列恢复融资租赁资产净额和负债，写出它们在合并表中的位置；再用 2026 到期表核回融资租赁负债.

<strong>解析. </strong>资产净额 $1,520-708=812$，包含在PP&E内；负债 $337+341=678$，分别包含在应计及其他流动负债、其他非流动负债内. 2026到期表 $569-34=535$ 与资产负债表分项 $275+260=535$ 相符.
...
<summary>跨公司租期与资源用途</summary>

Costco经营、融资租赁的加权剩余租期分别为20、25年，Salesforce分别为7、3年. 比较这两个报告日的合同组合，需结合设施用途、续约与替换安排，以及各期限内的付款分布. [^bf13-cost][^bf13-crm]
...
<strong>综合任务. </strong>用所选公司数据连接租赁负债与到期表，定位使用权资产，说明费用与现金的范围，并列出判断某项设施提前退出后果所需的材料.

<strong>参考解析. </strong>Costco经营与融资负债2,668、1,479，分别由未折现付款3,591、2,411减表列利息得到，融资租赁资产1,488列于其他长期资产；费用618与现金460的覆盖范围不同. Salesforce经营与融资负债2,737、535，分别与3,075−338、569−34相符，融资使用权净额614在PP&E内；列示费用614+375+25=1,014，特定范围现金1,087. 判断设施提前退出的剩余付款、转租回收及减值，需要具体合同和资产组资料.

[^bf13-fasb]: FASB ASU 2016-02, Section A，Summary 印刷 pp1–5／PDF pp7–11：承租人确认、两类租赁、可选期间、短期与可变付款及定义. [官方原件](https://storage.fasb.org/ASU%202016-02_Section%20A.pdf#page=7).
...
输入身份 `BFDE-SHARED-20260921-v1`. 公司金额单位为 USD M.
