...
识别测量对象、期间、单位、覆盖与版本，建立可比较的数据口径.

Entry: zh-ei02 | Node: EI-02 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
读者为有充分数学背景的高年级本科生至研究生. 教授EI-02. 先实际读取所涉原表和表注：
EIA Tables2.5–2.7：
...
先记录实际取得范围和版本，必读单元齐全后教学. 用混合图修复题诊断对象、单位、频率、存流量和版本；让读者用单位抵消重建约16.479美分/kWh，再区分季调与价格调整、平均价与边际价. 反馈指出哪项口径改变了比较含义.
...
```json
{
  ...
  "learning_task": "一个数列至少要说明**测量对象、期间、单位、覆盖和版本**；这些字段决定两个数字是否回答同一个问题.",
  "body_source": "body_markdown",
  "prompt": "读者为有充分数学背景的高年级本科生至研究生. 教授EI-02. 先实际读取所涉原表和表注：\nEIA Tables2.5–2.7：\nhttps://www.eia.gov/electricity/annual/html/epa_02_05.html\nhttps://www.eia.gov/electricity/annual/html/epa_02_06.html\nhttps://www.eia.gov/electricity/annual/html/epa_02_07.html\n制造材料直接读取Census May2026固定期次：\nhttps://www.census.gov/manufacturing/m3/historical_data/pressreleases/prel/2026/may26prel.pdf\n核第1页版次和发布日期、第3–4页定义、第6–9页表头/脚注；\n固定原件未取得时明确报告缺口，不以滚动“current”报告替代.\n涉及IEA图时读：\nhttps://iea.blob.core.windows.net/assets/7c671ef6-2947-4e87-beea-af0e1288e1d7/Electricity2025.pdf\npp57–61首段并看原图两个纵轴.\n\n先记录实际取得范围和版本，必读单元齐全后教学. 用混合图修复题诊断对象、单位、频率、存流量和版本；让读者用单位抵消重建约16.479美分/kWh，再区分季调与价格调整、平均价与边际价. 反馈指出哪项口径改变了比较含义.",
...
每份数据记录对象、地区、单位、频率、存流量、季节调整、价格调整、材料期、发布日期和版本. 缺失字段保持缺失.

EIA居民电价：$100\times244367/1482874\approx16.479$美分/kWh.

Census May2026于2026-07-02发布，5月初值、4月修订值. 出货与新订单为期间流量，未交订单与存货为期末余额，金额已经季调、未作价格调整. IEA事件图的功率与价格使用不同纵轴.
