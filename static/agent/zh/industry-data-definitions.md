...
## Supplied entry
一个数列至少要说明**测量对象、期间、单位、覆盖和版本**. 这五项共同决定两个数字是否回答同一个问题. 统计口径确认集中在本页；哪些指标值得选、怎样权衡证据，留给EI-15.
...
EIA 2024居民收入为244,367** M 美元**，电量为1,482,874** K MWh**. 因为 $1\ \mathrm{MWh}=1000\ \mathrm{kWh}$，所以电量的原始数值同样对应 M kWh. 先相除得到美元/kWh，再乘100变为美分/kWh：
...
要把“表格单元格能够精确读取”和“总体估计误差已经量化”分开. 该M3报告说明其调查面板**不是概率抽样**，因此不能据此计算通常意义上的抽样误差. Tables 1–4中工程机械这一行的公布数字仍可按表精确记录，但这只解决表值读取，不量化调查统计的不确定性.
