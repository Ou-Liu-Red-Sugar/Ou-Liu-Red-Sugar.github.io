...
最后一步反而增加 $0.52/0.46-1=13.0435\%$，因此该实验的频率比较并非单调改善. Table4 不是成本实验，不能用它推出“所有市场交易越频繁越差”；75 变 200 epochs 时还同时涉及数据集规模等变化，也不能识别单独增加 epoch 的因果效应.
...
[^htz-eq]: 同文 Eq.(3.8)，PDF8. 正文以 $t=0$ 的平坦方差曲线核对左右对象；该局部核对不等于完整模型对冲复现.
