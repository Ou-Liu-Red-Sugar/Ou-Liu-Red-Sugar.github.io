...
在恒定乘积与集中流动性模型中，比较LP、持币和自融资再平衡的库存、费用与损益.

Entry: zh-p33 | Node: P33 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
读者为有充分数学背景的高年级本科生至研究生. 先实际读取 agent_packet.required_readings 的全部指定单元，所选可选分支再读 optional_readings，核对版本、期间、单位及支持内容. 缺少必要单元时先取得等价可读原件，再进行依赖它的讲解. 按Figure3的0.2下节点重建四条LP、HODL及再平衡现金路径，证明凹性带来的LVR非负. 分别加入独立费用层和集中区间，跨上界后核单一Y库存；选读连续形式时读取Itô及真鞅条件. 先以完整推理任务诊断，再让读者计算和解释；按错误反馈，使用迁移题检验. 采用所列日期研究和教学输入，runtime_reading_log记录实际来源与范围.
...
```json
{
  "export_mode": "public",
  "entry_id": "zh-p33",
  "learning_task": "按Figure3的0.2下节点重建四条LP、HODL及再平衡现金路径，证明凹性带来的LVR非负. 分别加入独立费用层和集中区间，跨上界后核单一Y库存；选读连续形式时读取Itô及真鞅条件.",
  "required_readings": [
    {
      "source_id": "MOD-LVR",
      ...
      "version": "arXiv2208.06046v6，2026-08-20；PDFcurrent2026-05-03",
      "access": {
        "kind": "pdf_full_text",
        "uri": "https://arxiv.org/pdf/2208.06046v6",
        "verified_access_at": "2026-09-21"
      },
      "required_unit": {
        "locator": "§3资产/费用设置；§4自融资定义；§5.1完整有限树与Figure3；使用本文给出的凹性推导",
        "scope": "无费库存、同风险库存的自融资基准、结构LVR与独立费用层.",
        "purpose": "重建LP、持币和自融资基准，分离结构LVR、费用层与集中区间状态."
      },
      "supports": "无费库存、同风险库存的自融资基准、结构LVR与独立费用层.",
      "limits": "DD采用图3的. 2；重算PanelC库存；没有采用§7经验比例；理想参考市场不是现实可执行承诺."
    },
    {
      ...
      "required_unit": {
        "locator": "PDFpp1–3；§2/2.1与§3.1–3.2",
        "scope": "集中区间实际库存、出区间状态与费用独立记账.",
        "purpose": "重建LP、持币和自融资基准，分离结构LVR、费用层与集中区间状态."
      },
      "supports": "集中区间实际库存、出区间状态与费用独立记账.",
      "limits": "不声称本例边界为有效tick，不采用旧fee tier作当前完整目录."
    }
  ],
  "optional_readings": [
    {
      ...
      "required_unit": {
        "locator": "§4 Assumption1、Lemmas1–2及AppendixA.1/A.2完整相关证明",
        "scope": "无费库存、同风险库存的自融资基准、结构LVR与独立费用层.",
        "purpose": "选择连续时间推导时；§4及AppendixA.1/A.2"
      },
      "supports": "无费库存、同风险库存的自融资基准、结构LVR与独立费用层.",
      "limits": "DD采用图3的. 2；重算PanelC库存；没有采用§7经验比例；理想参考市场不是现实可执行承诺.",
      "branch": "continuous",
    ...
    "case_inputs": [
      {
        ...
        "identity": "Finite-tree LVR model plus separate concentrated-range approximation; not deployed-pool data.",
        "baseline_no_fee_lvr": {
          "fees": 0,
          "costs": 0,
          "paths": [
          ...
          "LVR": [
            0.05478012889165873,
            0.061938298108593415,
            0.1163977794943225,
            0.18917502950576204
          ],
          "expected_LP": 1.894427190999916,
          "expected_rebalancing": 2.0,
          "expected_LVR": 0.10557280900008417
        },
        "separate_fee_variant": {
          "fee_cash": 0.02,
          "cost": 0.005,
          "net_fee_cash": 0.015,
          "expected_total_lp_net": 1.909427190999916,
          "expected_rebalancing_minus_total_lp_net": 0.09057280900008391,
          "naming": "Do not relabel net shortfall as structural LVR."
        },
        "concentrated_range": {
          "bounds_P_Y_per_X": [
            0.64,
          ...
          "P_final": 1.69,
          "L": 5.454545454545458,
          "initial_x_X": 0.9090909090909094,
          "initial_y_Y": 1.0909090909090913,
          "final_x_X": 0,
          "final_y_Y": 2.1818181818181825,
          "lp_gross_Y": 2.1818181818181825,
          "hold_Y": 2.627272727272728,
          "fees_Y": 0.05,
          "costs_Y": 0.01,
          "lp_net_Y": 2.2218181818181826,
          "fees_required_to_match_Y": 0.45545454545454533,
          "status": "above range: inactive until re-entry"
        },
        "source_correction": {
          "tree": "§5.1 prose DD=.4 conflicts with Figure3 DD=.2; adopt explicit Figure3 .2.",
          "inventory": "Rebuild rebalancing inventory/cash from x*(P)=1/sqrt(P)."
        },
        "branch_boundaries": {
          "fee_layers": ".02/.005 finite-tree branch is separate from .05/.01 concentrated-range branch.",
          "empirics": "Do not cite unread §7 empirical proportions.",
          "ticks": "Range is continuous-price teaching approximation, not tick-valid deployed position."
        }
      }
    ],
    "operational_details": {},
    "static_default_result": {
      "unit": "Y; P in Y/X; x in X",
      "fee": false,
      "expectedLP": 1.894427190999916,
      "expectedR": 2,
      "expectedLVR": 0.10557280900008412,
      "expectedNetLP": 1.894427190999916
    },
    "attachments": [
      {
        "path": "https://ou-liu-red-sugar.github.io/notebook/labs/p-ijklm/shared/lp-self-financing-ledgers.csv",
        "branch": "core",
        "sha256": "e1cd072b8005a5be59031fbdfc0e08750c91753e1e462cf8b5cb6cb12c452ac8",
        "format": "csv",
        "public_access": "具名公开附件；与原稿字节一致."
      },
      {
        "path": "https://ou-liu-red-sugar.github.io/notebook/labs/p-ijklm/shared/lp-range-grid.csv",
        "branch": "core",
        "sha256": "62c729e58b368e35d526356452474ec81ecad64ac45aaea8b81a9974b73e5a66",
        "format": "csv",
        "public_access": "具名公开附件；与原稿字节一致."
      }
    ],
...
[
  {
    ...
    "static_equivalent": {
      "figures": [
        "https://ou-liu-red-sugar.github.io/notebook/labs/p-ijklm/figures/P33-a.svg",
        "https://ou-liu-red-sugar.github.io/notebook/labs/p-ijklm/figures/P33-b.svg"
      ],
      "body_tables": true,
      "event_data": [
        {
...
## Sources
- [Automated Market Making and Loss-Versus-Rebalancing](https://arxiv.org/pdf/2208.06046v6): 无费库存、同风险库存的自融资基准、结构LVR与独立费用层.
...
[
  ...
  {
    "from": "p33-rebalancing",
    "relation": "supported_by",
    "to": "MOD-LVR",
    "reason": "无费库存、同风险库存的自融资基准、结构LVR与独立费用层.",
    "locator": "§3资产/费用设置；§4自融资定义；§5.1完整有限树与Figure3；使用本文给出的凹性推导",
    "scope": "无费库存、同风险库存的自融资基准、结构LVR与独立费用层."
  },
  {
  ...
  },
  {
    ...
    "relation": "supported_by",
    "to": "MOD-LVR",
    "reason": "选择连续时间推导时；§4及AppendixA.1/A.2",
    "locator": "§4 Assumption1、Lemmas1–2及AppendixA.1/A.2完整相关证明",
    "scope": "选择连续时间推导时；§4及AppendixA.1/A.2",
    "branch": "continuous"
  },
  {
    "from": "p33-lab",
    "relation": "illustrated_by",
    "to": "EXP-P33-LP",
    "reason": "重建LP、持币和自融资基准，分离结构LVR、费用层与集中区间状态."
  }
]
```
