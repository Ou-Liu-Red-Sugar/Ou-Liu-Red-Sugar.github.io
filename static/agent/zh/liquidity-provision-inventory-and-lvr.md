...
Entry: zh-p33 | Node: P33 | Language: zh | Editorial revision: 2026-09-21

## Teaching instructions
请先实际读取本篇必读原件的指定完整单元，再围绕“重建LP、持币和自融资基准，分离结构LVR、费用层与集中区间状态.”带我完成学习. 先让我自己判断方向、对象或基准，再让我逐步重建一项承重计算；不要先把答案全部说出. 选读分支只有我选中后才启用对应原件和输入. 每次解释都分清真实规则、教学假设、作者报告与本站复算. 逐事件核现金、负债、费用和剩余仓位；资金不足时停止已完成结果. 使用本包正文里的完整解析反馈我哪里错、为什么、如何迁移. 未来runtime_reading_log由你实际读取后填写，不能把作者或支持者日志当成自己的阅读. 不要接触账户或更新冻结数据.
...
```json
{
  "export_mode": "public",
  "entry_id": "zh-p33",
  "learning_task": "重建LP、持币和自融资基准，分离结构LVR、费用层与集中区间状态.",
  "required_readings": [
    {
      "source_id": "MOD-LVR",
...
- [Uniswap v3 Core](https://app.uniswap.org/whitepaper-v3.pdf): 集中区间的真实/虚拟储备与有限交换；手续费增长公式以固定代码作局部单位校正.
