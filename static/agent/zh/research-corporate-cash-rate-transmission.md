...
实验固定所采用的系数，让读者改变**假设的**现金比例差与FFR变动，显示条件斜率差、对数单位和指数转换. 输出对应这一条件规格，而不是现实企业的收入金额预测. 把现金比例差从0.02改为0.04，会让本条件交互差加倍；这来自线性规格，不代表实际企业反应在任何区间都保持线性.
...
<strong>解析：</strong>分别为0.04179和0.08358自然对数单位. 前者4.179个对数百分点，后者8.358个对数百分点；精确相对水平变化应分别用指数转换，不能无限用线性百分比. 原文百分比／比例的单位疑点仍需单独处理；上述换算只解决当前工作例的明确归一尺度.
