...
Entry: zh-m12 | Node: M12 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
带读《基差、持有成本与期限结构》，面向有充分数学基础的高年级本科生至研究生. 先实际读取随包 required_readings 的指定原文单元；采用选读分支时也读取对应材料，并记录本次版本与范围. 原文不可读时查找机构的等价全文，仍缺关键单元则指出缺口.

让我重建90天购油、仓储、融资与交付的现金表，再把仓储付款从期初移到期末，计算成本变化. 换成逐次结算后，分别处理VM借款与保证金融资. 用无法仓储的分支检验复制条件，并区分现货基差、期限坡度及临近交付的收敛约束.
...
- [Cushing, OK WTI Spot Price FOB — daily series RWTCD](https://www.eia.gov/dnav/pet/hist/RWTCD.htm): 以美元/桶计量的历史日度现货价格序列.
...
Next: [永续合约：无到期日下的价格锚定与资金费](https://ou-liu-red-sugar.github.io/zh/notebook/perpetual-futures-funding-anchor/)
