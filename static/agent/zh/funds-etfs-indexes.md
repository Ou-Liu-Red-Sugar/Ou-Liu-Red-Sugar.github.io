...
Entry: zh-m08 | Node: M08 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
用M08《基金、ETF与指数》带我分析份额权利和申赎机制. 先通过本篇 reference 包中的公开链接，实际读完 required_readings 指定单元，记录来源、版本、读取范围和支持当前讲解的内容. PDF 表格与公式核对原页. 读取失败时从原机构查找同版材料；仍缺关键单元则指出受影响的任务，补齐后再讲解. 根据一次完整任务诊断我的水平，跳过已掌握的步骤；反馈先给定位提示，再展开解析.
先读VTI的2026年4月概要招募书与7月补充文件，区分基金、ETF份额类别、指数及跟踪策略，说明各自名称和策略安排. 让我画出份额持有、底层资产、二级交易和AP申赎关系. 再用单类别教学基金推导NAV=(净资产)/(同类别份额)，以全基金3M与ETF类别10K份检验分子分母匹配. 按公允价值申购，从10K增至60K份，再赎回50K份，说明单位净值和持有比例如何变化.
使用每单位50K份的实物篮子教学设定，按ETF bid/ask与篮子买卖成本重建双向现金差额，逐项检查资格、窗口、同步报价、深度和先行资金. 将使用中间价算出的6,500改算为3,500，分解3K差异，再比较每份费用0.19和深度20K的变式. 解释运营费、跟踪差、执行成本及现金分配，收益计算中各项只计一次. 选读FCA OP68时先读§2、§4.1–4.2及图注，讨论2018–2022样本如何区分库存和直接申赎；讨论估计结果时补读模型、变量及对应结果. 最后换一组报价检验我能否独立重建NAV、两方向现金和执行条件.
...
## Sources
- [Vanguard Total Stock Market ETF — Summary Prospectus](https://www.sec.gov/Archives/edgar/data/36405/000003640526000197/f44849d1.htm): VTI 的 ETF 份额类别、0.03% 费率、假设年回报 5% 的费用示例、抽样复制、二级市场交易和授权参与人权限.
...
- [Updated Investor Bulletin: Exchange-Traded Funds (ETFs)](https://www.investor.gov/introduction-investing/general-resources/news-alerts/alerts-bulletins/investor-bulletins-24): SEC 的 ETF 投资者教育材料：主动与指数基金、NAV、零售交易和授权参与人申赎、买卖价差. 50,000 份是材料使用的通用申赎单位示例.
- [Morningstar Market Indexes](https://indexes.morningstar.com/morningstar-market-indexes): Morningstar 市场指数的覆盖范围、市值分段与实施设计.
- [ETF (Mis)pricing — FCA Occasional Paper 68](https://www.fca.org.uk/publication/occasional-papers/op68-etf-mispricing.pdf): 研究AP库存与ETF定价，连接一级申赎记录和二级交易报告. 样本为2018年1月至2022年10月的128只主要在LSE交易的实物ETF，包括50只股票型和78只债券型.
