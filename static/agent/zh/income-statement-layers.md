...
Entry: zh-bf06 | Node: BF-06 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
围绕BF-06《利润表：收入、成本与利润的层次》开展教学，对象是有微积分、线性代数及基本概率基础、未系统学过会计的高年级本科至研究生. 先确认所选行业分支和当前任务. 在讲解前，使用读取工具实际取得agent_packet中common及所选branch的全部required_readings完整单元（含表头、脚注和条件）. optional_readings只有选中相应研究/扩展时才转为必读. 记录标题、版本、实际范围以及支持当前教学的关键设定；runtime_reading_log初始为空，不以编辑访问或参考清单冒充本次已读. 访问失败时尝试同版本官方等价正文；仍缺失就指出具体缺口，不拿摘要代替承重单元. 诊断任务：先选择零售或银行，让读者从完整原表重建至少两层利润. 沿当前同源正文、静态表和supplied_inputs逐段推进，先让读者分析再反馈，已会步骤直接跳过. 判断理解的尺度：按所选分支判断：零售能用公司MD&A的30026毛利接回10383经营利润；银行能由193341−97898构成NII并扣除信用计提. 最后迁移：默认在所选行业换读上一年度，从同一原表重建两层利润并解释口径；若读者选择另一行业，再先读该分支全部指定原件，说明哪些中间指标不能直接沿用. 保留历史观测与教学设定身份、单位、期间、未解释差额.
...
```json
{
  ...
  "required_readings": [
    {
      ...
      "access": {
        "kind": "html_full_text",
        "uri": "https://openstax.org/books/principles-financial-accounting/pages/6-6-describe-and-prepare-multi-step-and-simple-income-statements-for-merchandising-companies"
      },
      "required_unit": {
        "locator": "§6.6全文",
      ...
      "supports": "商品企业的多层利润结构；不作为银行的默认模板.",
      "branch": "retail",
      "required_when_selected": false,
      "title": "Principles of Accounting, Volume 1: Financial Accounting — §6.6 Describe and Prepare Multi-Step and Simple Income Statements for Merchandising Companies",
      "authors": [
        "Mitchell Franklin",
        "Patty Graybeal",
        "Dixon Cooper"
    ...
    },
    {
      ...
      "required_unit": {
        "locator": "完整Consolidated Statements of Income p.37；MD&A Gross Margin p.27；Note1 Membership Fees pp.47–48",
        "scope": "所列完整单元、相关表头、脚注及当前计算所需说明",
        "purpose": "原表、公司定义毛利与会员费确认"
      },
  ...
  "optional_readings": [
    {
      ...
      "required_unit": {
        "locator": "完整Consolidated Statements of Income印刷p.165/PDF197；业务Introduction p.46",
        "scope": "所列完整单元、相关表头、脚注及当前计算所需说明",
        "purpose": "银行利润的经营对象与GAAP层次"
      },
  ...
  "diagnosis": "先选择零售或银行，让读者从完整原表重建至少两层利润.",
  "feedback_criteria": "按所选分支判断：零售能用公司MD&A的30026毛利接回10383经营利润；银行能由193341−97898构成NII并扣除信用计提.",
  "transfer_task": "默认在所选行业换读上一年度，从同一原表重建两层利润并解释口径；若读者选择另一行业，再先读该分支全部指定原件，比较不能直接沿用的中间指标.",
...
资本投入与现金变化见[现金流量表](https://ou-liu-red-sugar.github.io/zh/notebook/cash-flow-statement/).
...
交互算法：成本扣减按公司原表层级；Cost gross=sales−merchandise，operating=gross+membership−SG&A. JPM NII=interest income−expense，再加全部非息收入，减计提、非息费用和税.

期间：Costco FY2023为53周，FY2024和FY2025为52周；银行按其报告年度读取.

静态结果：https://ou-liu-red-sugar.github.io/notebook/labs/bf-bc/static.html#EXP-BF06-PROFIT-LAYERS.

## Experiment inputs and static equivalents
```json
[
  {
    "id": "EXP-BF06-PROFIT-LAYERS",
    "title": "利润表：收入、成本与利润的层次：交互实验",
    "anchor": "bf06-experiment",
        ...
        "cost_income": [
          {
            ...
            "pretax": 10818,
            "net": 8099,
            "gross_margin_percent": 11.124366460179614
          },
          {
            "gross": 27267,
            "revenue": 254453,
            ...
            "pretax": 9740,
            "net": 7367,
            "gross_margin_percent": 10.923184777165748
          },
          {
            "gross": 25124,
            "revenue": 242290,
            ...
            "pretax": 8487,
            "net": 6292,
            "gross_margin_percent": 10.56918093475243
          }
        ],
        "jpm_income": [
          {
...
## Sources
- [Principles of Accounting, Volume 1: Financial Accounting — §6.6 Describe and Prepare Multi-Step and Simple Income Statements for Merchandising Companies](https://openstax.org/books/principles-financial-accounting/pages/6-6-describe-and-prepare-multi-step-and-simple-income-statements-for-merchandising-companies): 商品企业利润表的单步式与多步式结构，包括销售收入、销售成本、毛利及期间费用.
...
[
  ...
  },
  {
    ...
    "locator": "§6.6完整单元",
    "scope": "商品企业的多层利润结构；不作为银行的默认模板."
  },
  {
    "from": "bf06-retail",
    "relation": "compares_with",
    "to": "bf06-bank",
    "reason": "商品经营与资金经营的利润层次"
  },
  {
    "from": "bf06-experiment",
    "relation": "illustrated_by",
    "to": "EXP-BF06-PROFIT-LAYERS",
    "reason": "沿零售和银行各自的经营关系读完整利润表，重建利润层次并解释两期变化.",
    "at_section": "bf06-experiment",
    "conditions": "三个历史年度不能当预测；52/53周保留；不计算跨行业统一毛利排名."
  }
]
```
...
拆一份企业财报: step 3/14
选零售或银行，用完整原表重建利润形成的两至三层关系.
利润采用应计期间；下一篇把同一经营过程接到实际现金收付.
Next: [现金流量表：经营、投资与筹资](https://ou-liu-red-sugar.github.io/zh/notebook/cash-flow-statement/)
