...
Entry: zh-p14 | Node: P14 | Language: zh | Editorial revision: 2026-09-21

## Teaching instructions
你正在教授P14《P14 现金担保看跌：有条件地买股，不是带利息的限价单》. 先确认读者选择的分支. 实际打开required_readings的公开原文，完整读取指定单元和必要表图；记录标题、版本、位置及其支持的具体步骤. optional_readings只有在分支被选择时转为必读. 若只有摘要、目录或访问失败，不能声称完成，也不凭印象补课；可复用本会话已完整读过的相同版本单元并注明.

然后一次给出一项完整诊断任务：把9500总交割款与净取得成本分开；同K105重建平价，再比较95限价已成交与put未指派的110终点. 不逐个考四则运算. 使用本包全文与PF-GRID-01唯一冻结链，先让读者解释关系，再计算，最后使用本篇迁移题. 区分真实契约、模型生成mid、合成执行价和假设事件. 反馈标准：不能以9358代替付款额；9000及时现金缺500须停止，CSP95不得拿去冒充CC105同K复制.
...
```json
{
  ...
  "learning_task": "把9500总交割款与净取得成本分开；同K105重建平价，再比较95限价已成交与put未指派的110终点.",
  "required_readings": [
    {
      "source_id": "PFH-CSP",
...
另设一条无费用、无利息的教学路径：D10的95限价买单已经实际成交100股，而95 put没有提前指派. 两个事件直接作为路径输入；市场触及95本身不能推出订单一定成交，也不能推出put一定被指派.
...
阅读该文件3.0版§3.1时还需做零利率校验：印刷式(4)在零利率时给出1，若再代入式(3)的“1加该值”，会让余额翻倍. 因此这对式子不作为本例的利息计算依据；规则文件在这里用于账户、滚动结构和PUT策略组成，上节现金例则从终点支付独立推导.
...
[
  ...
  },
  {
    ...
    "locator": "Description至Comments/Related Position的完整策略主文",
    "scope": "接股意愿与义务、现金保留、未取得股票的机会成本；不把净成本等同总交割款."
  },
  {
    "from": "p14-goal",
    "relation": "supported_by",
