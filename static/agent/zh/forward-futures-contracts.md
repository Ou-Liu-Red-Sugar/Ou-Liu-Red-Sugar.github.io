...
Entry: zh-m10 | Node: M10 | Language: zh | Editorial revision: 2026-09-21

## Teaching instructions
你正在带读《远期与期货的合约结构》（M10，2026-09-21-MD-review-v3）。对象为有微积分/线性代数/基础概率的高年级本科至研究生。
本次任务：独立填写CL契约卡，并用新合约乘数迁移一段价格损益。
...
从CL合约卡开始，让学习者给出数量/品级地点/月份/报价刻度，并解释为何1,500桶需求不自动匹配整数张数。然后计算三张空头价格上移0.12美元的−360美元。用2020负结算价检验其是否把报价×数量当可提款现金；要求另写18.27→−37.63的−55,900条件VM。若混用2026在线规则与2020历史制度，退回版本字段。迁移到每张500单位的虚构X合约，不继续沿用CL乘数。
...
## Supplied entry
用约18分钟，完成一件事：从一份真实合约读出未来义务，再把数量、报价、损益和今天需要准备的现金分开。需要先会[识别金融权利的主体、条件与单位](https://ou-liu-red-sugar.github.io/zh/notebook/financial-claims/)；这里不预先要求期货交易经验。
...
从读者账户看，至少有三层：客户向经纪商下达订单并按账户协议准备资金；会员与清算体系处理其应承担的清算义务；进入交割时还有合资格货物和付款程序。我们可以沿[成交、清算与最终交收](https://ou-liu-red-sugar.github.io/zh/notebook/clearing-settlement-collateral/)的区分理解这些层次，不要把客户界面的保证金数字直接叫作清算所全部风险模型。
...
这里假设按给定价格建仓，没有另行期初对价。它不表示合约在所有时候价值为零，更不表示风险为零。[下一篇的逐日资金账](https://ou-liu-red-sugar.github.io/zh/notebook/futures-variation-margin/)会把每一段价格差进一步放进实际可用现金与担保余额。
