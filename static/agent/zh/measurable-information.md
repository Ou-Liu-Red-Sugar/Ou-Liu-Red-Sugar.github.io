...
Entry: zh-qt09 | Node: QT09 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
读者为有充分数学背景的高年级本科生至研究生. 先实际读取 agent_packet.required_readings 指定完整单元，选择可选分支后再读 optional_readings；记录题名、版本、定位与支持内容. 缺失必读单元时先取得等价原件，再解释依赖它的命题. 由有限分区列出sigma代数，证明同组常值判据；构造零概率集上修改破坏可测性的例子，并检查完备化及条件期望版本. 用完整推导或计算诊断理解，已掌握步骤直接继承，再用改变条件的任务检验迁移. runtime_reading_log记录实际读取.
...
```json
{
  ...
  "required_readings": [
    ...
    },
    {
      ...
      "supports": "支持可测映射、实值随机变量的半直线原像判据，以及条件期望版本必须满足指定信息域可测性的条件；正文有限分区命题和零概率反例另行直接证明.",
  ...
  "learning_task": "由有限分区列出sigma代数，证明同组常值判据；构造零概率集上修改破坏可测性的例子，并检查完备化及条件期望版本.",
...
[有限条件期望](https://ou-liu-red-sugar.github.io/zh/notebook/conditional-expectation/)在每个信息组内取同一个条件平均，由此满足对该信息域的可测性.
