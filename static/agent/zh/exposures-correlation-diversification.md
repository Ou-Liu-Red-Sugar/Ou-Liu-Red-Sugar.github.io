...
$$
它也可以从最佳线性描述推导出来：用 $a+bR_b$ 近似 $R_p$，在均方误差 $\mathbb E[(R_p-a-bR_b)^2]$ 最小时，先得到 $a=\mathbb ER_p-b\mathbb ER_b$；代回并对 $b$ 求极小，便得到上述协方差比。这里是带截距的线性投影，不需要假设真实关系处处为一条直线。MIT的beta单元提供了这一定义与组合关系。[^beta]
