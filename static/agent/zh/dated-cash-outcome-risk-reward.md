...
Entry: zh-p31 | Node: P31 | Language: zh | Editorial revision: 2026-09-21

## Teaching instructions
你正在教授P31《日期现金、结果分布与Risk-Reward》，读者具有高年级本科至研究生数学背景.
...
本篇任务与反馈尺度：
先让读者列−253.71和2029末一股售价两笔现金，核1200/365.25；企业救援发行不是旧股东强制追加. 由全概率/全期望公式接四代表点，保留近似身份；重算361.428923、11.372500%及8.347799%，并用非线性/Jensen解释T>1的方向. 严格亏损的等号边界须说清. 用第二类80/120的练习证明条件均值可能保留ETW却丢失亏损概率；当前参数区间没有全类支持身份. 实验只在4定价×4权重16项内切换，具名经营替代另回P04. Tail损失84.4023不是最坏5%平均. 概率翻转固定原价格和Base/Tail；16.886105pp不是通用隐含概率. 最后用一年末股息再投资题检验日期现金和不重复计利.
...
第一个式子把每类内的分布混合起来，第二个式子是全期望公式；期望公式要求财富可积. 实际研究可能还不能识别每类的完整分布，于是只提供代表点 $w_s$. 此时 $\widehat{\mathbb{E}}[W_T]=\sum_sp_sw_s$ 是**四点近似**；除非代表点确为条件均值，否则不要把近似的帽子悄悄去掉.
...
$$
r_{\mathbb{E}W}=\left(\frac{361.428923090674}{253.71}\right)^{1/3.285420944559}-1
=11.372500\%.
$$
