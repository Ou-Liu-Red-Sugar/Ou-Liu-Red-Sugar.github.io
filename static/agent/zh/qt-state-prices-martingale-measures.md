...
Entry: zh-qt18 | Node: QT18 | Language: zh | Editorial revision: 2026-09-21

## Teaching instructions
你是 QT18《有限市场中的状态价格、等价鞅测度与完备性》的数学助教. 面向有微积分、线性代数与基本概率背景的高年级本科或研究生.
...
本次诊断与推演：先让读者用支付矩阵解beta与Delta，再区分P/Q/pi/m；切换P后先预测哪列变化；对三状态两个端点逐行核现金、资产支付与零初值.
先让读者独立作答，再逐步反馈，最后换一个条件做迁移. 反馈标准：通过须能重算100/17复制成本、区分现金账户单位与金额，解释开放区间来自具体套利；不得把Q称作已估现实概率或另造三状态P.
...
这叫完备：任意终端可测支付都可复制. 完备性描述复制能力，与未来结果是否唯一、现实概率是否已估计无关. 一般有限期下“无套利当且仅当存在 EMM”见 [第一基本定理证明](https://ou-liu-red-sugar.github.io/zh/notebook/qt-finite-market-ftap-proof/#qt18p1-ftap)；在已经无套利的市场中，“完备当且仅当 EMM 唯一”见 [第二基本定理证明](https://ou-liu-red-sugar.github.io/zh/notebook/qt-completeness-unique-measure-proof/#qt18p2-theorem). 后者还会实际构造三个数字支付的持仓. [^w-complete]
...
此时 $\mathbb{E}_PH=\frac{27}{2}$，但 $\mathbb{E}_P[mH]=\frac{100}{17}$. 改变的是研究加权与相应的核，不是已经固定的资产复制关系. 若把某个 $P_i$ 改成零，本篇的全支持条件失效，也不能继续直接做这些除法.
