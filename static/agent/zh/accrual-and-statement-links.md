...
期末权益比期初减少30，其中20来自亏损，10来自股东分配. 权益变动与利润之间还需计入所有者交易，真实原表见[权益和股数](https://ou-liu-red-sugar.github.io/zh/notebook/equity-shares-and-shareholder-claims/).
...
**解析**. 还缺收款条款. 如果交付即使收款权无条件成立，只待到期付款，1月10日应列应收；如果已交付的对价还要等第二项独立交付完成，可能先形成合同资产. [收入、应收与合同余额](https://ou-liu-red-sugar.github.io/zh/notebook/revenue-receivables-contract-balances/)会专门处理这个界线.
...
合同事件按日期排列；同日中间状态按履约、权利成立、开票、收款呈现. R为累计履约，U为已成立的无条件收款权，C为已收现金；AR=max(U−C,0)，X=R−max(U,C)，CA=max(X,0)，CL=max(−X,0).

改变权利成立日期代表合同权利事实改变；单独改变开票日期保持原收款权条件. 可分别改变履约、权利成立、开票、收款和观察日期，比较状态变化.

静态结果：https://ou-liu-red-sugar.github.io/notebook/labs/bf-bc/static.html#EXP-BF04-ACCRUAL-TIMELINE.
