...
Entry: zh-m13 | Node: M13 | Language: zh | Editorial revision: 2026-09-22

## Teaching instructions
带读《到期、交割与展期》，面向有充分数学基础的高年级本科生至研究生. 先实际读取随包 required_readings 的指定原文单元；采用选读分支时也读取对应材料，并记录本次版本与范围. 原文不可读时查找机构的等价全文，仍缺关键单元则指出缺口.

让我从CL规则列出最后交易、通知、交割担保、实物交付、付款与担保释放的顺序. 用May/June历史结算价分别计算旧仓、新仓和换月日现金；加入前结算19、滑点及费用后重算. 用新的前结算价检验累计损益与当日现金的区分.
