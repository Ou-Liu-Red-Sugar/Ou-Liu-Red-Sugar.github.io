...
PF-GRID-01的价格是合成教学链：95 put mid1.42，105 call mid1.57，利率与分红先设零；每张每次费0.65. XYZ只作为教学标的；真实美式实物契约的身份与交付单位取自OCC. 先用不含摩擦的mid结构建立账户，再加入bid/ask和费用，并把模型价格与实际成交记录区分开.
