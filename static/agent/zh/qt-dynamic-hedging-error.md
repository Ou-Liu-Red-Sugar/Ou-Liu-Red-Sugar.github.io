...
共同随机数只减少比较中无关的抽样差异，不承担从 $P$ 到 $Q$ 的换测度论证. QT17 的一年、漂移 $0.08$ 示例属于另一生成模型；本账本的价格路径始终按漂移 $r=0.03$ 的 $Q$ 模型生成.
...
无费用时，较细网格在这组实验中使 RMSE 下降. 加入成本后，这种改善不再单调：5 bps 下 256 区间的 RMSE 高于 64 区间；20 bps 下频繁调仓的费用更加明显. 这些结果只比较当前固定的策略、资产、借贷限制和成本机制；因此 64 或 16 区间只是该实验中的候选表现，不能外推为真实市场最优频率.
...
这一分支需要 [Itô 公式](https://ou-liu-red-sugar.github.io/zh/notebook/qt-ito-formula-gbm/)，验证的是此模型中这一份 call 的连续复制；一般连续市场是否完备还需要额外条件.
...
[^xsp]: Cboe，*XSP Options Product Specification*，Underlying、Multiplier、Exercise Style、Settlement of Option Exercise；[官方规则](https://www.cboe.com/tradable_products/sp_500/mini_spx_options/specifications)，2026-09-21 核对. 该规则不确定券商 booking 的精确时点.
[^cost]: Stephen Boyd、Kasper Johansson、Ronald Kahn、Philipp Schiele、Thomas Schmelzer，*Markowitz Portfolio Construction at Seventy*，2024-01-05 公开稿，§2.2–2.4、§3.4–3.5、§4.1–4.3；[原文](https://web.stanford.edu/~boyd/papers/pdf/markowitz.pdf). 这里采用成本、预测和约束对象的定义.
[^pricing]: 正文的 Gaussian 积分与 PDE 推导用于这份 call 的复制；一般 Itô 公式及其条件见 [完整证明单元](https://ou-liu-red-sugar.github.io/zh/notebook/qt-ito-formula-proof/). 价值相等与自融资条件分别核对.
