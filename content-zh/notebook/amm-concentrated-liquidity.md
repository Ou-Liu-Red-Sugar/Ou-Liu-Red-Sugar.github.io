...
这里要分开两个停止原因：**用户的price limit不允许继续向前**，与**当前区间流动性已经耗尽、跨tick后需要换用新的L**. 两者都能让本段计算停止，但规则含义不同.
...
这一步只讨论一次兑换的金额和交换后的库存. 长期提供流动性的回报还要加入路径、费用、成本及比较基准；“相对一直持币的变化”和“相对自融资再平衡的变化”是两个不同问题，一次交换的手续费只是长期LP账本中的一项.
