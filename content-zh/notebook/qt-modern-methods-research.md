{
  "title": "现代金融学习方法：把一项研究读到可以复核",
  "description": "分别重建 KMZ 实证设计或 HTZ 架构比较，进行指标/对象/证据边界审计。",
  "layout": "entry",
  "notebookid": "zh-qt25",
  "math": true,
...
这里 $h_k$ 不是股票数量，而是网络内部记忆。各时点的 $F_k$ 可以不同；论文所谓充分循环结构，不等于所有时点必须共享同一个网络参数。隐状态可能学习历史的有用压缩，却不自动等于 <a class="inline-ref" href="/zh/notebook/qt-markov-state/" data-reference="zh-qt13">Bayes 模型中的统计充分状态<span aria-hidden="true"> ↗</span></a>。
