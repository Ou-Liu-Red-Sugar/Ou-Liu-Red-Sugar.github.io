...
这里的“账户维持保证金率”是平台定义的账户风险比率，不能拿下一节公式里的0.5%档位要求代替. 组合模式也不是“给每个仓显示一个参考强平价”：原表明确为不适用. 全仓与组合的计算依赖完整账户状态；本节只解释它们的触发准则，单仓数值例只用于逐仓公式.
...
保险机制处理的是极端市场中，亏损超过个别头寸可承受部分后如何承接系统损失；它不改变客户头寸本身的固定合约支付. 合约支付、资金能力和系统损失承接应分开读取.
