...
从这里到估计问题只差一步：$\widehat F_n$ 随样本变化，$\widehat{\operatorname{\mathbb{E}S}}_p$ 也会变化.<a class="inline-ref" href="/zh/notebook/estimation-prediction-uncertainty/" data-reference="zh-qt19">估计误差与预测不确定性<span aria-hidden="true"> ↗</span></a>将区分这种抽样不确定性和未来结果本身的波动；本节先把要估的对象算准确.
